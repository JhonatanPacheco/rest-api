...
class AuthModel extends CI_Model {
     ...
     public function __construct()
   {
    ...
    public function validateToken(){
      ...
      if($response['status'] != REST_Controller::HTTP_OK){
        //$this->response($response, $response['status']);
        //return json_output(401,array('status' => $response['status'],'message' => 'Unauthorized.'));
    ...
    public function login($username,$password)
    {
        ...
        if($q == ""){
            return array('status' => 409,'message' => 'Username not found.');
        } else {
            $hashed_password = $q->password;
            $id              = $q->id;
            if (hash_equals($this->encryption->decrypt($hashed_password), $password)){

               $last_login = date('Y-m-d H:i:s');
               $token = crypt(substr( md5(rand()), 0, 70),'data');
               ...
               // echo "Wrong password";
               // exit();
               return array('status' => 409,'message' => 'Wrong password.','errorCode'=>'ERRPWD');
            }
        }
    }
    ...
    public function auth(){
        ...
        if($q == ""){
            return array('status' => 401,'message' => 'Authentication required.');
        } else {
    ...
    public function create($user,$password,$name){
      
      //$this->db->insert('users',array('username' => $user,'password' => $this->encryption->encrypt($password),'name' => $name));
       // if ($this->db->trans_status() === FALSE){
       //    $this->db->trans_rollback();
       //    return array('status' => 500,'message' => 'Internal server error.');
       // } else {
